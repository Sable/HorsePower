...
L q16_peephole_0(V *z, V y, V *x){
    ...
    initV(z2,vp(x2),lenZ);
    initV(z3,vp(x3),lenZ);
    DOT(len, if(vB(y,i)){L c=offset[tid]++;vQ(z0,c)=vQ(x0,i);vQ(z1,c)=vQ(x1,i);vLL(z2,c)=vLL(x2,i);vLL(z3,c)=vLL(x3,i);}) R 0;
}
// Pattern FP3
// Pattern FP4
L q16_peephole_1(V z, V x, V y){
    ...
    DOP(r0, {V t=vV(y,i); L len=vn(t); L tot=0; B f[199]={0};\
        DOJ(len, if(!f[j]){ \
        DOK(len, if(k!=j && vLL(x,vL(t,j))==vLL(x,vL(t,k))) f[k]=1)\
...
E compiled_main(){
    ...
    PROFILE( 10, t27, pfnMember(t27, t22, t26));
    ...
    PROFILE( 25, t68, pfnMember(t68, t64, t9));
