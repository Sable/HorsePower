...
I promoteValueWithIndex(V z, V x, IL typMax, V ind){
    IL size = ind?vn(ind):vn(x);
    IL *indx = ind?sL(ind):NULL;
...
B checkMatch(V x){
    switch(xp){
        caseV if(xn>0){IL n=vn(vV(x,0)); DOI(xn,if(n!=vn(vV(x,i)))R 0)} break; /* List */
    }
    R 1;
}
 ...
 * _ -> .  // single
 */
static S genLikeString(S inStr, IL inLen){
    S outStr=NULL;
    IL outLen= inLen;
...
/* error code -2 */
static IL decideType(IL x, IL y){
...
/* typ init. -1 */
I getCommonType(V x, I *typ, IL *len){
    if(isList(x)){
...
static L binarySearch(LL *x, L low, L high, LL val){
    L mid = (low+high)>>1;
    if(x[mid] == val) R mid;
    else if(low == mid) R -1;
...
I searchOrdered(V z, V x, V y){
    ...
    if(isInt(x)){
        L lenZ = vn(y);
        initV(z,H_L,lenZ);
        DOP(lenZ, {L k=binarySearch(sI(x),0,vn(x),vI(y,i)); vL(z,i)=k<0?vn(x):k;})
    }
...
I listFlat2Normal(V z, V x){
    V g2 = vg2(x); IL typ2 = vp(g2);
    if(isTypeGroupBasic(typ2)){
        IL lenZ = vn(x), typSize = getpTypeSize(typ2); G cur=vg(g2); B f0 = typ2==H_C;
        initV(z, H_G, lenZ);
        DOI(lenZ, {V t=vV(z,i); IL k=vIL(x,i); IL kSize=getTypeSize(typ2,k)-f0; initV(t,typ2,k);\
