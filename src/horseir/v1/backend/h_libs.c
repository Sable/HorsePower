...
static IL find_hash(HN ht, IL htMask, void* src, void* val, IL valI, IL typ){
    IL hashKey = getHashValue(val,valI,typ) & htMask;
    HN t = hV(ht,hashKey);
    while(hN(t)){
        t = hN(t); IL td = hD(t);
        switch(typ){
...
static L insertHash(HN ht, IL htMask, void* src, L srcI, void* val, L valI, I typ){
    R insert_hash(ht,htMask,src,srcI,typ,true);
}

static L insertHashMany(HN ht, IL htMask, void* src, L srcI, void* val, L valI, I typ){
...
#define lib_index_template(typ) \
    HN hashT; \
    IL hashLen  = getHashTableSize(sLen); \
    IL hashMask = hashLen - 1; \
    L hashCur = getHashHeap(); P("size1 = %lld, %lld\n",hashLen,hashCur); \
    CHECKE(createHash(&hashT,hashLen)); \
    DOI(sLen, insertHash(hashT,hashMask,src,i,NULL,-1,typ)) \
    DOI(vLen, {IL t=find_hash(hashT,hashMask,src,val,i,typ);targ[i]=t<0?sLen:t;}) \
    setHashHeap(hashCur);

I lib_index_of_B(IL* targ, B* src, IL sLen, B* val, IL vLen){
    IL flag[2]={-1}; I c=0;
    DOI(sLen, if(c<2 && -1==flag[src[i]]){flag[src[i]]=i;c++;if(2==c)break;})
    DOP(vLen, {IL t=flag[val[i]]; targ[i]=t<0?sLen:t;})
    R 0;
}

I lib_index_of_J(IL* targ, J* src, IL sLen, J* val, IL vLen){
    lib_index_template(H_J);
    R 0;
}

I lib_index_of_H(IL* targ, H* src, IL sLen, H* val, IL vLen){
    lib_index_template(H_H);
    R 0;
}

I lib_index_of_I(IL* targ, I* src, IL sLen, I* val, IL vLen){
    lib_index_template(H_I);
    R 0;
}

I lib_index_of_L(IL* targ, L* src, IL sLen, L* val, IL vLen){
...
I lib_index_of_F(IL* targ, F* src, IL sLen, F* val, IL vLen){
    lib_index_template(H_F);
    R 0;
}

I lib_index_of_E(IL* targ, E* src, IL sLen, E* val, IL vLen){
    lib_index_template(H_E);
    R 0;
}

I lib_index_of_C(IL* targ, C* src, IL sLen, C* val, IL vLen){
    IL buff[256];
    memset(buff,-1,256*sizeof(IL));
    DOI(sLen, if(buff[src[i]]==-1) buff[src[i]]=i)
    DOI(vLen, {L t=buff[val[i]]; targ[i]=-1==t?sLen:t;})
    R 0;
}

I lib_index_of_X(IL* targ, X* src, IL sLen, X* val, IL vLen){
    lib_index_template(H_X);
    R 0;
}

I lib_index_of_S(IL* targ, S* src, IL sLen, S* val, IL vLen){
    lib_index_template(H_S);
    R 0;
}

I lib_index_of_G(IL* targ, V src, IL sLen, V val, IL vLen){
...
void lib_quicksort_char(IL *rtn, V val, IL low, IL high, B *isUp, FUNC_CMP(cmp)){
    S str = sC(val);  IL len = high-low;  B f0 = isUp?*isUp:1; //isUp?1:*isUp;
    if(len < 10){
        DOI3(low, high, DOJ3(low, high, if(str[rtn[i]] > str[rtn[j]]) { L t=rtn[i]; rtn[i]=rtn[j]; rtn[j]=t; } ))
    }
    else {
        P("entering large char sort: len = %lld\n", len);
        IL cnt[256]={0}, step[256]={0}, cursor=0;
        IL *temp = (IL*)malloc(sizeof(IL)*len);
        ...
        memcpy(rtn+low, temp, sizeof(IL)*len);
        free(temp);
    }
    if(!f0){ // desc order
...
static L lib_partition(L *rtn, V val, L low, L high, B *isUp, FUNC_CMP(cmp), B *leftSame){
    ...
    while(true){
        while(++i && i<high){
            IL temp = (*cmp)(val,rtn[i],rtn[pivot],isUp);
...
IL lib_quicksort_cmp(V val, IL a, IL b, B *isUp){
    DOI(vn(val), {V t=vV(val,i); B f=isUp?isUp[i]:1; \
                  L x=lib_quicksort_cmp_item(t,a,b,&f); if(x!=0)R x;})
    R 0;
}

IL lib_quicksort_cmp_item(V t, IL a, IL b, B *isUp){
    B f = isUp?*isUp:1;
    // switch(vp(t)){
    ...
    if(low < high - 1){
        if(low + 1 == high - 1){
            ...
            if((*cmp)(val,rtn[mid-1],rtn[mid],isUp)>0){ // need to sort
                ...
                memcpy(rtn+low,LARGE_BUFF,sizeof(IL)*len);
...
static IL lib_mergesort_decide(V val, IL a, IL b, B *isUp, FUNC_CMP(cmp)){
    DOI(vn(val), {IL k=(*cmp)(vV(val,i),a,b,isUp+i); if(k!=0) R k; })
    R (a>b?1:a<b?-1:0);
}

static void lib_mergesort_list(IL *rtn, V val, IL low, IL high, B *isUp, FUNC_CMP(cmp)){
    // P("Entering merge sort: (%lld, %lld)\n",low,high);
    // #define SWAP_I(a,b) { L temp=rtn[a]; rtn[a]=rtn[b]; rtn[b]=temp; }
    #define SWAP_I(a,b) { rtn[a]^=rtn[b]; rtn[b]^=rtn[a]; rtn[a]^=rtn[b]; }
    // P("type of val: %lld\n", vp(val));
    ...
    if(low < high - 1){
        if(low + 1 == high - 1){
            ...
            IL mid = (low + high) >> 1, len = high-low;
 ...
 *     : false ->  1 (desc)
 */
static B lib_order_by_sorted(IL *targ, V val, B *isUp, IL low, IL high, FUNC_CMP(cmp)){
    IL rtn = 0;
    #define DOIm(m, n, x) {for(IL i=m+1,i2=n;i<i2;i++)x;}
    DOIm(low, high, {IL t=(*cmp)(val,targ[i-1],targ[i],isUp); \
    ...
    if(isOrder && !(*isUp)){ // reverse order
        //P("low = %lld, high = %lld\n", low,high); getchar();
        DOI3(low, (low+high)/2, \
            {IL temp=targ[i]; IL pos=high-(i-low)-1; targ[i]=targ[pos]; targ[pos] = temp;})
    }
    R isOrder;
}

void lib_order_by_list(IL *targ, V val, B *isUp, IL tLen, IL colId, FUNC_CMP(cmp)){
    DOP(tLen, targ[i]=i);
...
void lib_quicksort_list(L *targ, V val, B *isUp, L low, L high, L colId, FUNC_CMP(cmp)){
    ...
    // gettimeofday(&tv0, NULL);
    {
        ...
        do{
            // find next
            IL start = pos, end = high;
            IL hend = start + 5, hpos = -1; // heuristic end/pos
...
static I lib_get_group_by_other(V z, V val, IL* index, L iLen, IL (*cmp)(V,IL,IL,B*)){
    IL k, c, cz; V d,t;
    ...
    V zKey = getDictKeys(z);
    V zVal = getDictVals(z);
    initV(zKey, H_IL, lenZ);
    initV(zVal, H_G , lenZ);
    c=iLen>0?1:0;
    k=cz=0;
    d=vV(zVal,k++);
    if(iLen>0) vIL(zKey,cz++)=index[0];
    DOIa(iLen, if(0!=(*cmp)(val,index[i-1],index[i],NULL)){ \
                 vIL(zKey,cz++)=index[i]; \
                 initV(d,H_IL,c); d=vV(zVal,k++); c=1; } \
               else c++;)
    if(c>0) initV(d,H_IL,c);
    /* 3. fill indices into each cell */
    // P("step 3\n");
    k=0, c=0;
    d=vV(zVal,k++);
    if(iLen>0) vIL(d,c++)=index[0];
    DOIa(iLen, if(0!=(*cmp)(val,index[i-1],index[i],NULL)){ \
                  d=vV(zVal,k++); vIL(d,0)=index[i]; c=1; } \
               else vIL(d,c++)=index[i])
    // P("exit\n");
    R 0;
}
 ...
 * index[x] => x
 */
static I lib_get_group_by_order(V z, V val, IL* index, IL iLen, IL (*cmp)(V,IL,IL,B*)){
    L k, c, cz; V d,t;
    /* 1. get the total number of cells: lenZ */
    // P("step 1\n");
    IL lenZ=iLen>0?1:0;
    ...
    if(iLen>0) vIL(d,c++)=0;
    DOIa(iLen, if(0!=(*cmp)(val,i-1,i,NULL)){ \
                  d=vV(zVal,k++); vIL(d,0)=i; c=1; } \
               else vIL(d,c++)=i)
    // P("exit\n");
    R 0;
}

I lib_get_group_by(V z, V val, IL* index, IL iLen, IL (*cmp)(V,IL,IL,B*)){
    // #ifdef OPT_Q1
...
static B lib_member_fast1(void* src, void* val, IL valI, IL typ){
    switch(typ){
        LIB_MEMBER_F1(B);
        LIB_MEMBER_F1(J);
        LIB_MEMBER_F1(H);
...
static B lib_member_fast2(void* src, void* val, IL valI, IL typ){
...
time_tic();
    ...
    IL parZ[H_CORE], offset[H_CORE]; 
    memset(parZ  , 0, sizeof(IL)*H_CORE);
    memset(offset, 0, sizeof(IL)*H_CORE);
    ...
    DOT(vLen, if(tempK[i]>=0){IL p=offset[tid]; \
         if(compareOpRow(x,y,tempK[i],i,f)){vIL(z0,p)=tempK[i];vIL(z1,p)=i;} \
         HI t0=tempH[i]; while(t0){p++; if(compareOpRow(x,y,tempK[i],i,f)){vIL(z0,p)=t0->ival;vIL(z1,p)=i;}t0=t0->inext;} \
         offset[tid]=p+1; })
elapsed=time_toc();
