...
#define FUNC_CMP(x) IL (*x)(V,IL,IL,B*)
...
IL   lib_quicksort_cmp        (V val, L a, L b, B *isUp);
IL   lib_quicksort_cmp_item   (V t  , L a, L b, B *isUp);
void lib_order_by_list        (L *targ, V val, B *isUp, L tLen, L colId, FUNC_CMP(cmp));
void lib_quicksort_list       (L *targ, V val, B *isUp, L low , L high, IL colId, FUNC_CMP(cmp));
