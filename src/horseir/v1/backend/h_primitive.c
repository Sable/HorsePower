...
I pfnFlip(V z, V x){
    if(isDict(x)){
        ...
        initList(z, cellSize);
        DOJ(cellSize, {\
            V y=vV(z,j); initV(y,H_IL,vn(x)); \
            DOP(vn(x), { vIL(y,i)=vIL(vV(x,i),j); }) })
    }
    else R E_DOMAIN;
    R 0;
}
...
static I fetchColumnValue(V z, V x, V y, B f){
    if(isTable2(x) && isSymbol(y)){
        ...
        if(isOne(y)){
            ...
            //P("lenZ = %lld\n", lenZ);
            DOI(lenZ, { \
               IL colIndex = findColFromTable(x, vQ(y,i)); \
        ...
        if(isTypeGroupReal(vp(x)) && isTypeGroupReal(vp(y))){
            ...
            // }
                ...
                switch(typCell){
                    caseB {
                        switch(typMax){
                            caseB DOI(lenX, DOJ(lenY, if(OuterOp(op,vB(tempX,i),vB(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                            caseJ DOI(lenX, DOJ(lenY, if(OuterOp(op,vJ(tempX,i),vJ(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                            caseH DOI(lenX, DOJ(lenY, if(OuterOp(op,vH(tempX,i),vH(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                            caseI DOI(lenX, DOJ(lenY, if(OuterOp(op,vI(tempX,i),vI(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                            caseL DOI(lenX, DOJ(lenY, if(OuterOp(op,vL(tempX,i),vL(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                            caseF DOI(lenX, DOJ(lenY, if(OuterOp(op,vF(tempX,i),vF(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                            caseE DOI(lenX, DOJ(lenY, if(OuterOp(op,vE(tempX,i),vE(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                            caseC DOI(lenX, DOJ(lenY, if(OuterOp(op,vC(tempX,i),vC(tempY,j))){vL(vV(z,0),c)=i;vIL(vV(z,1),c)=j;c++;})); break;
                        }
                    } break;
                    default: R E_DOMAIN;
                }
            ...
            if(4 == op || 5 == op){
                ...
                DOI(lenZ, initV(vV(z,i),H_IL,c))
                switch(typCell){
                    caseB {
                        switch(typ){
                            caseQ DOI(lenX, DOJ(lenY, if(OuterOp(op, vQ(x,i),vQ(x,j))){vL(vV(z,0),c)=i; vIL(vV(z,1),c)=j; c++;})); break;
                        }
                    } break;
                    default: R E_DOMAIN;
                }
...
I pfnSubString(V z, V x, V y){
    if(isString(x) && isLong(y)){
        if(2==vn(y)){
            IL start = vIL(y,0); // starting position
            IL seg   = vIL(y,1); // length of segment
            initV(z,H_S,vn(x));
            // warning: must not DOI -> DOP, because of race condition in allocStrMem
            DOI(vn(x), {S t=vS(x,i); IL t0=strlen(t); S p=allocStrMem(seg); \
                       IL space = t0 - start; IL len=seg<space?seg:space;\
