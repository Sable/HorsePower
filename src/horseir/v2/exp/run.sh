...
usage(){
    printf '%s\n' \
      ...
      "   $1 src <id> [runs]  ## ./make.sh <id> opt source && ./run-query.sh <id> [runs]" ""\
...
if [ "$#" -eq 2 ]; then
    mode=$1; qid=$2
    if [ ${mode} = "src" ]; then
        (set -x && ./make.sh ${qid} opt source && ./run-query.sh ${qid} 1)
    elif [ ${mode} = "exp" ]; then
        ...
        (set -x && ./make.sh ${qid} cur source && ./run-query.sh ${qid} 1)
    else
        usage $0
    fi
elif [ "$#" -eq 3 ]; then
    mode=$1; qid=$2; runs=$3
    if [ ${mode} = "src" ]; then
        (set -x && ./make.sh ${qid} opt source && ./run-query.sh ${qid} ${runs})
    elif [ ${mode} = "exp" ]; then
