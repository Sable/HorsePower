...
L q20_peephole_1(V *z, V y, V *x){
    ...
    initV(z1,vp(x1),lenZ);
    initV(z2,vp(x2),lenZ);
    DOT(len, if(vB(y,i)){L c=offset[tid]++;vL(z0,c)=vL(x0,i);vL(z1,c)=vL(x1,i);vE(z2,c)=vE(x2,i);}) R 0;
}
L q20_peephole_2(V *z, V y, V *x){
    ...
    initV(z0,vp(x0),lenZ);
    initV(z1,vp(x1),lenZ);
    DOT(len, if(vB(y,i)){L c=offset[tid]++;vQ(z0,c)=vQ(x0,i);vQ(z1,c)=vQ(x1,i);}) R 0;
}
...
E compiledQ20(){
    ...
    // Variable t133 has type H_L and len 909455
