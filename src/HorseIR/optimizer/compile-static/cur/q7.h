...
L q7_peephole_0(V *z, V y, V *x){
    ...
    initV(z3,vp(x3),lenZ);
    initV(z4,vp(x4),lenZ);
    DOT(len, if(vB(y,i)){L c=offset[tid]++;vL(z0,c)=vL(x0,i);vL(z1,c)=vL(x1,i);vE(z2,c)=vE(x2,i);vE(z3,c)=vE(x3,i);vD(z4,c)=vD(x4,i);}) R 0;
}
...
E compiled_main(){
    ...
    // Variable t105 has type H_L and len 121324
