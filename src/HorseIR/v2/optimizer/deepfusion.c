...
static void genCodeDeep    (gNode *rt, B isRT);
static void genCodeDeepList(gNodeList *list, I size);
static void genCodeDeepListNode(gNode *rt, L id);
...
static void genCodeDeep(gNode *rt, B isRT){
    ...
    if(isRT){
        ...
        SP(temp, "q%d_deepfusion_%d",qid,phTotal++);
    ...
    if(isKindS(rt)){
        ...
        glueAny("%s(", getFuncNameDeep(nodeName2(fn)));
        List *params = getNodeParams(n);
...
static void genCodeDeepListParam1(List *list, gNode *rt, L id){
    I total = rt->pnum;
    ...
    DOIa(total, {gNode *t=rt->pnode[total-i-1]; \
            if(i>1) glueChar(','); \
            if(t) genCodeDeepListNode(t, id); \
            else genCodeBody(rtn[i], t); })
    // TODO: free rtn
}

static void genCodeDeepListLen(gNodeList *list, L id){
    gNode *rt = list->g;
    if(list->numLast > 0){
...
static void genCodeDeepListInit(gNode *rt, L id, S initStr){
...
static void genCodeDeepListBody(Node *p, gNode *g, L id){
    if(g) genCodeDeepListNode(g, id);
    else {
        if(instanceOf(p, nameK)){
            genCodeName(p, searchName(varNames, nodeName2(p)));
        }
...
static void genCodeDeepListNode(gNode *rt, L id){
    ...
    if(isKindR(rt)){
        ...
        genCodeDeepListBody(getNodeParams(n)->val, rt->pnode[0], id);
    }
    else if(isKindE(rt) || isKindD(rt) || isKindB(rt)){
        Node *fn = getEachFuncNode(n);
        //P("-------\n"); printNode(fn); getchar();
        glueAny("%s(", getFuncNameDeep(nodeName2(fn)));
        genCodeDeepListParam1(getNodeParams(n), rt, id); // skip 1st one
        glueChar(')');
    }
    else TODO("Add support kind: %c", rt->kind);
}
 ...
 */
static void genCodeDeepListMain(gNodeList *list, L id){
    gNode *rt = list->g;
    if(isKindM(rt)){
        genCodeDeepListLen(list, id); // search for last stmt
    }
    else genCodeDeepListNode(rt, id);
}
...
static void genCodeDeepList(gNodeList *list, I size){
    ...
    if(size > 0){
        ...
        DOI(size, {gNode *x=plist->g; C temp[99]; \
                genCodeDeepListInit(x,i,temp); \
                P("temp = \"%s\"\n", temp); getchar(); \
                genCodeDeepListMain(plist,i); \
                P("code = \"%s\"\n", code); getchar(); \
                plist=plist->next;})
    }
}
...
static void genCodeListFusion(gNodeList *x){
    ...
    while(a){
        if(!flag[c]) {
            ...
            genCodeDeepList(tmp->next, size); // gen code
